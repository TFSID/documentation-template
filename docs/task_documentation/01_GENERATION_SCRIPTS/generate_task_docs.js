...
// Function untuk generate template markdown content
...
## Tujuan
- [Tujuan utama 1 yang ingin dicapai]
- [Tujuan utama 2 yang ingin dicapai]
- [Tujuan utama 3 yang ingin dicapai]
...
```
[Kebutuhan teknis dan fungsional untuk tugas ini]
```

### 2. Desain Solusi
```
[Arsitektur dan desain solusi]
```
...
${techStack.map(tech => `- **${tech.category}**: ${tech.items}`).join('\n')}
...
## Next Steps
Setelah ${task.id} selesai, lanjut ke [taskID berikutnya] untuk implementasi [deskripsi].
...
function getTechStackByModule(moduleName) {
  const baseStack = [
    { category: 'Frontend', items: 'React.js/Vue.js dengan TypeScript' },
    { category: 'Backend', items: 'Node.js/Python dengan Express.js/FastAPI' },
    { category: 'Database', items: 'PostgreSQL/MongoDB' },
    { category: 'UI Framework', items: 'Tailwind CSS/Ant Design' }
  ];

  switch(moduleName) {
    case 'Paket Request':
      return [...baseStack, { category: 'File Upload', items: 'React Dropzone + AWS S3' }];
    case 'Program Umrah':
      return [...baseStack, { category: 'Calendar', items: 'FullCalendar.js' }];
    case 'Pendaftaran Haji':
      return [...baseStack, { category: 'Integration', items: 'REST APIs + SOAP untuk government' }];
    case 'Konsultasi EduTrip':
      return [...baseStack, { category: 'Video', items: 'WebRTC + Zoom SDK' }];
    case 'Profil Management':
      return [...baseStack, { category: 'Security', items: 'bcrypt + JWT' }];
    case 'Manajemen Jamaah':
      return [...baseStack, { category: 'Real-time', items: 'Socket.io + WebSocket' }];
    case 'Manajemen Visa/Paspor':
      return [...baseStack, { category: 'OCR', items: 'Tesseract.js + OpenCV' }];
    case 'Manajemen Identitas':
      return [...baseStack, { category: 'Biometric', items: 'Face-api.js + TensorFlow.js' }];
    default:
      return baseStack;
  }
}
...
${(() => {
  ...
  const avgHours = Math.round(totalHours / tasksData.length);
  const maxHours = Math.max(...tasksData.map(task => task.estimated_hours));
  const minHours = Math.min(...tasksData.map(task => task.estimated_hours));
  
  return `
- **Total Hours**: ${totalHours} hours
- **Average Hours**: ${avgHours} hours per task
- **Maximum Hours**: ${maxHours} hours
- **Minimum Hours**: ${minHours} hours
